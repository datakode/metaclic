...
uDataUtils.urlify = function (text) {
    ...
    uData = function (obj, options) {
        ...
        var addSpatialZoneMap = function (dataset) {
                var bloc = obj.find('.dataset-result[data-dataset="' + options.dataset + '"]'),
                    zones_li = bloc.find("ul.spatial_zones li");
            ...
            addPreviewMap = function (dataset_id, datasetdata) {
                var bloc = obj.find('.dataset-result[data-dataset="' + dataset_id + '"]'),
                    geojson_links = bloc.find('.resources-list a[data-format="JSON"],.resources-list a[data-format="GEOJSON"]');
                geojson_links.each(function () {
                    var geojson_link = jQuery(this),
                        map_title = geojson_link.data("map_title"),
                        resource_id = geojson_link.data("id"),
                        geojson_url = geojson_link.prop("href");
        ...
        var initMap = function () {
                ...
                for (var i in options.background_layers) {
                    ...
                    if ("string" == typeof bl)
                        if (void 0 != uDataUtils.baseLayers[bl]) bl = uDataUtils.baseLayers[bl];
                        else try {
                            bl = eval(bl);
                        } catch (err) {
            ...
            addResource = function (resource, data_dataset) {
                jQuery.each(data_dataset.resources, function (k, val) {
                    ...
                    jQuery.getJSON(resource.data.url, function (data) {
                        if (data.features.length > featurelength_limit) return obj.find(".geojson_loading_" + resource.id).slideUp("fast"), !1;
                        ...
                        if ("string" == typeof resource.template)
                            if (jQuery(resource.template).length) resource.template = Handlebars.compile(jQuery(resource.template).first().html());
                            else try {
                        ...
                        return layer.addTo(map), loadedLayers[resource.title] = layer, updateBBoxAndLayerController(),
                            obj.find("ul.resources").append(Templates.li_resource(resource)), obj.find(".geojson_loading_" + resource.id).slideUp("slow"), !0;
                    }).fail(function (data) {
            ...
            loadResource = function (resource) {
                ...
                if (resource = jQuery.extend({}, resource_defaults, resource || {}), null != datasetdata && datasetdata.id == resource.dataset) addResource(resource, datasetdata);
                else {
    ...
    var API_ROOT = "https://www.data.gouv.fr/api/1/",
        contentlength_limit = 25e5,
        icons_limit = 200,
        featurelength_limit = 2e5,
        checklibs = function () {
            var dependences = {
        ...
        start = function () {
            ...
            _uData.lang = lang = "fr", i18n.init({
                ...
                var res = "",
                    nPage = Math.ceil(total / page_size);
                if (1 == nPage) return "";
                ...
                if (jQuery("div.uData-data").length) {
                    ...
                    container.on("click", "a[data-page]", function (e) {
                        ...
                        var tag = jQuery(this).data("removetag"),
                            tags = _uData.container.data("tags"),
                            index = jQuery.inArray(tag, tags);

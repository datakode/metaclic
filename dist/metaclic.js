...
MetaclicUtils.Templates.li_resource = [ '<li data-id="{{id}}">', '<a href="{{metadata_url}}">{{title}}</a>', '<i class="fa fa-copyright"></i> {{_ license}}', '<p class="organization" data-id="{{organization.id}}" data-slug="{{organization.slug}}">', '<img alt="{{  organization.name }}" src="{{ organization.logo }}">', "<span>{{organization.name}}</span>", "</p>", "</li>" ], 
...
jQuery(document).ready(function($) {
    ...
    Metaclic = function(obj, options) {
        ...
        }), options.organizationList = options.organizationList.join(","), "" == options.organization && _Metaclic.orgs[0] && (options.organization = _Metaclic.orgs[0].id);
        ...
        _Metaclic.displayLastDatasets = function() {}, _Metaclic.displayDatasets = function() {
            ...
            if (void 0 === options2.sort && (options2.sort = sortTypes[0].id), "string" == typeof options2.sort && (options2.sort = options2.sort.replace("-", ""), 
            sortDesc && (options2.sort = "-" + options2.sort)), delete options2.organizations, 
            delete options2.sharemaps, delete options2.baseUrl, delete options2.organizationList, 
            void 0 != options2.tags && (options2.tag = options2.tags), !options2.organization) {
                var params = {
                    q: options.q,
                ...
                return obj.html(html), updateGeozonesTrans(), updateListLimit(), scrollTop(), !1;
            }
            ...
            url = url.replace(/tag%5B%5D/g, "tag"), jQuery.getJSON(url, function(data) {
                ...
                var html;
                html = options.generator ? Templates.organizationAdd(params) + Templates.shareCode(options) + Templates.datasetsForm(params) + Templates.datasets(data) : Templates.datasetsForm(params) + Templates.datasets(data), 
                obj.html(html), updateGeozonesTrans(), updateListLimit(), scrollTop();
            }).fail(function() {
        ...
        var addSpatialZoneMap = function(dataset) {
            ...
            if (zones_li.each(function() {
                ...
                var map = L.map("map" + options.dataset + "_zones", {
                    scrollWheelZoom: !1
                }).setView([ 0, 0 ], 1), layers = _Metaclic.container.data("background_layers");
                layers = layers ? layers.split(",") : [ "OSM-Fr", "Positron", "Outdoors_OSM", "Hydda" ];
                var default_layer, baseMaps = {}, attribution = ' &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors';
                for (var a in layers) MetaclicUtils.baseLayers[layers[a]] && ("OSM-Fr" != MetaclicUtils.baseLayers[layers[a]].title ? baseMaps[MetaclicUtils.baseLayers[layers[a]].title] = L.tileLayer(MetaclicUtils.baseLayers[layers[a]].url, {
                    attribution: attribution
                }) : (default_layer = L.tileLayer(MetaclicUtils.baseLayers[layers[a]].url, {
                    attribution: attribution
                }), default_layer.addTo(map), baseMaps[MetaclicUtils.baseLayers[layers[a]].title] = default_layer));
                map.addControl(new L.Control.Layers(baseMaps)), jQuery.getJSON(url, function(data) {
            ...
            obj.find('.dataset-result[data-dataset="' + dataset_id + '"]').find('.resources-list a[data-format="JSON"],.resources-list a[data-format="GEOJSON"]').each(function() {
        ...
        var _MetaclicMap = {}, layers = _Metaclic.container.data("background_layers");
        layers = layers ? layers.split(",") : [ "OSM-Fr", "Positron", "Outdoors_OSM", "Hydda" ];
        var defaults = {
            title: !1,
            sharelink: !1,
        ...
        _MetaclicMap.addBackground = function(title, layer, show) {
            backgroundLayers[title] = layer, !0 === show && layer.addTo(map), updateBBoxAndLayerController();
        };
    ...
    var API_ROOT = "https://www.data.gouv.fr/api/1/", contentlength_limit = 25e5, icons_limit = 200, featurelength_limit = 2e5, checklibs = function() {
        ...
        for (var i in dependences) void 0 === window[i] && (0 == jQuery('script[src="' + dependences[i] + '"]').length && jQuery('<script src="' + dependences[i] + '"><\/script>').appendTo("body"), 
        ready = !1);
        ...
        _Metaclic.lang = lang = "fr", i18n.init({
            ...
            return url || baseUrl + "../img/placeholders/" + type + ".png";
            ...
            if (value) {
                var EXCERPT_TOKEN = "\x3c!--- --- --\x3e";
                void 0 === length && (length = 128);
                var text, ellipsis;
                return value.indexOf("\x3c!--- excerpt --\x3e") && (value = value.split(EXCERPT_TOKEN, 1)[0]), 
        ...
        container.length) {
            ...
            container.each(function() {
                var obj = jQuery(this);
                Metaclic(obj, obj.data()).displayLastDatasets();
            });
            var loadDataSets = function() {
                container.each(function() {
                    var obj = jQuery(this);
                    Metaclic(obj, obj.data()).displayDatasets();
                });
            };
            ...
            if (jQuery("div.Metaclic-data").length) {
                var container = jQuery("div.Metaclic-data"), setPage = function(p) {
                    ...
                    if (void 0 === organization) {
                        var options = _Metaclic.container.data(), exp = /,/g;
                ...
                container.on("click", "a[data-page]", function(e) {
                    ...
                    e.preventDefault(), $("datalist").empty();
                    var research = e.target.value, research_uri = encodeURIComponent(research).replace(/[!'()]/g, escape).replace(/\*/g, "%2A"), url = API_ROOT + "organizations/suggest/?q=" + research_uri + "&size=10", jqxhr = $.get(url);
                    jqxhr.done(function(result) {
                        last_research_list = result, result.forEach(function(element) {
                    ...
                    var organizations = ($(this).val(), container[0].dataset.organizations);
                    ...
                    last_research_list.forEach(function(element) {
                        if (research == element.name) {
                            $("datalist").empty(), last_research_list = [], $('input[name="research"]').val("");
                            var is_unique = !0;
                            if (organizations.forEach(function(organization) {
                                organization == element.id && (is_unique = !1);
                            }), is_unique) {
                                organizations.push(element.id), 1 == _Metaclic.orgs.length && "" == _Metaclic.orgs[0].id && (_Metaclic.orgs = []), 
                                _Metaclic.orgs.push(element), container[0].dataset.organizations = organizations.toString();
                                var exp = /,/g, organization = container[0].dataset.organizations.replace(exp, "|");
                    ...
                    e.preventDefault();
                    var id = e.target.dataset.id, organizations = container[0].dataset.organizations;
                    organizations = organizations.split(",");
                    for (var k = 0; k < organizations.length; k++) organizations[k], organizations[k] == id && organizations.splice(k, 1);
                    for (var k = 0; k < _Metaclic.orgs.length; k++) _Metaclic.orgs[k], _Metaclic.orgs[k].id == id && _Metaclic.orgs.splice(k, 1);
                    ...
                    for (var unique = !0, index = 0; index < tags.length; index++) {
                        tags[index] == tag && (unique = !1);
                    }
                    unique && (tags.push(tag), _Metaclic.container.data("tags", tags));
        ...
        jQuery.getJSON(url, function(data) {
            for (var i in _Metaclic.orgs) {
                _Metaclic.orgs[i].id == data.id && (_Metaclic.orgs[i] = data);
            }
